...
harmony_bcodes  = all_healthy@assays$RNA@data@Dimnames[[2]]
harmony_ids 	= numeric()
pulp    		= c(2,3,13,14)

# Science Cloud
data_folder 	 = "/IMCR_shares/Moorlab/Laura/Data/Tooth/Pulp"
merged_spliced 	 = list()
merged_unspliced = list()

for (p in 1:length(pulp)) {

	i 						= pulp[p]
	# Find all of the current pulp's bcodes 
	label 					= paste0('pulp', i)
	# barcodes for current sample
	curr_bcodes 			= harmony_bcodes[harmony_bcodes %like% label]
	# ids in harmony (important in case we don't use all samples and therefore certain cells that went into harmony)
	harmony_ids 			= c(harmony_ids, which(harmony_bcodes %like% label))
	# trimmed barcodes to remove the pulp prefix added by Seurat for matching to bcodes from bustools' outputs "spliced" and "unspliced"
	...
	# find id of all bcode matches in this pulp's s.barcodes.txt files to find ids to use on s.mtx matrix
	# curr_s_bids 				= which(label_all_s_bcodes$spliced %in% bcodes) 
	curr_s_bids 				= which(bcodes %in% label_all_s_bcodes$spliced) 
...
orig_bcodes 			= harmony_bcodes[harmony_ids] # that's to exclude last sample (pulp7)
...
embeds = Embeddings(all_healthy[["umap"]])[harmony_ids,]

# save umap embedding
write.csv(embeds, file = file.path(output_dir, "embednonames.csv"), row.names = FALSE)

# save workspace
save.image(file = file.path(data_folder, "pulp_vel.RData"))
